...
Modbus.prototype.openSerial = function (serialPath, baudrate, parity) {
  ...
  this.master.on('error', function (err) {
...
Modbus.prototype.openTcp = function (host, port) {
  this.master = h5Modbus.createMaster({
    ...
    connection: new h5Modbus.TcpConnection({ host: host, port: port }),
    suppressTransactionErrors: false,
    retryOnException: true,
    maxConcurrentRequests: 1,
    defaultUnit: 0,
